import { h } from '../bottom-sheet.core.js';
...
const mix$1 = calc.getValueFromProgress;
class BottomSheet {
    constructor() {
        this.subscriptions = [];
        this.dragging = false;
    ...
    draggingChanged() {
        const action$$1 = this.dragging ? disableBodyScroll : enableBodyScroll;
        action$$1(this.containerEl);
    }
    ...
    componentDidLoad() {
        ...
        this.setBoundariesHeight();
        this.sheetStyler = index(this.sheetEl);
        ...
        listen(this.sheetEl, 'mousedown touchstart').start(() => {
            ...
            const applyOverdrag = v => {
                if (v < 0)
                    return mix$1(0, v, tug);
                if (v > max)
                    return mix$1(max, v, tug);
                return v;
            };
            this.pointer = index$1$1({ y: this.sheetY.get() })
                .pipe(({ y }) => y, applyOverdrag);
            this.pointer.start(this.sheetY);
        });
        listen(document, 'mouseup touchend').start(() => {
            this.dragging = false;
            this.intertia = index$1({
                min: 0,
...
class BottomSheetScreen {
    ...
    static get style() { return ".sc-bottom-sheet-screen-h{display:block;position:absolute;top:0;right:0;bottom:0;left:0;z-index:calc(var(--sheet-z-index, 9) - 1);background-color:var(--sheet-screen-background,rgba(0,0,0,.4))}"; }
}

export { BottomSheet, BottomSheetIndicator, BottomSheetScreen };
